...
func EnsureOpenAPIModule(cfg Config) error {
	...
	if !cfg.AutoAdd {
		return fmt.Errorf("user module must require github.com/fox-gonic/openapi; run `go get github.com/fox-gonic/openapi` or pass --auto-add")
	}
	add := exec.Command("go", "get", "github.com/fox-gonic/openapi")
	add.Dir = cfg.Workdir
...
func hasOpenAPIRequire(workdir string) (bool, error) {
	...
	for _, req := range file.Require {
		if req.Mod.Path == "github.com/fox-gonic/openapi" {
			return true, nil
		}
	}
	return false, nil
