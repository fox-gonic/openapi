...
仓库 `openapi/` 子目录是独立 Go module `github.com/fox-gonic/openapi`（用 `replace` 指向本地路径），已实现：
...
```
openapi/                            # standalone library + CLI module
├── go.mod
├── openapi.go                      # 反射核心
├── ...                             # 其它 library 反射逻辑
├── cmd/
...
CLI 与 library 在**同一 module**：CLI 直接 `import "github.com/fox-gonic/openapi"` 复用 generator；driver 模板里 import 的也是这个 module。
...
- **目标**：用户文档 + 迁移指南
- **交付**：
  - `cmd/fox-openapi/README.md` — quickstart
