...
- Read handler and struct field comments with `openapi.Source([]string{"."})`
...
The example is its own Go module and imports `github.com/fox-gonic/openapi`
through a local `replace` so it can run from this checkout.
