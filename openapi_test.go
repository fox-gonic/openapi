...
import (
	...
	v1users "github.com/fox-gonic/openapi/internal/collisionfixture/v1/users"
	v2users "github.com/fox-gonic/openapi/internal/collisionfixture/v2/users"
)
...
type ambiguousStatusResponse struct {
	status  int
...
func createDocumentedUserNoContent(_ *fox.Context, _ documentedCreateUserRequest) (statusResponse[documentedUserResponse], error) {
	return statusResponseWithStatus(http.StatusNoContent, documentedUserResponse{}), nil
}
...
func TestGenerateReadsFieldLineCommentsFromSource(t *testing.T) {
	...
	schemaName := "openapi_test_lineCommentResponse"
...
func TestGenerateCombinesFieldDocAndLineCommentsFromSource(t *testing.T) {
	...
	schemaName := "openapi_test_combinedCommentResponse"
	props := spec["components"].(map[string]any)["schemas"].(map[string]any)[schemaName].(map[string]any)["properties"].(map[string]any)
...
func TestGenerateOmitsBodyForInferredNoContentStatusResponse(t *testing.T) {
	engine := fox.New()
	engine.POST("/status-users", createDocumentedUserNoContent)
	...
	require.Equal(t, "No Content", noContent["description"])
	require.NotContains(t, noContent, "content")
...
func TestGenerateUsesInferredStatusWhenStatusWrapperBodyIsAmbiguous(t *testing.T) {
	engine := fox.New()
	engine.POST("/status-users", createDocumentedUserAmbiguousStatus)
	...
	accepted := responses["202"].(map[string]any)
...
func TestGenerateUsesRegisteredFormatters(t *testing.T) {
	...
	schemaName := "openapi_test_customIDResponse"
	props := spec["components"].(map[string]any)["schemas"].(map[string]any)[schemaName].(map[string]any)["properties"].(map[string]any)
...
func TestGenerateUsesCustomErrorSchema(t *testing.T) {
	...
	require.Equal(t, "#/components/schemas/openapi_test_customErrorResponse", schema["$ref"])

	components := spec["components"].(map[string]any)["schemas"].(map[string]any)
	require.Contains(t, components, "openapi_test_customErrorResponse")
}
